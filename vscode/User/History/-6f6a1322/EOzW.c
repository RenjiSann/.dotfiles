...
int main(int argc, char **argv) {
    ...
    if (write(opts.pipe[1], "OK", 2) == -1) {
        die(1, "Failed to send 'OK' to pipe");
    }
    if (close(opts.pipe[1]) == -1) {
        die(1, "Failed to close pipe");
    }
...
static int child_main(void *data) {
    ...
    // Before initializing file system, wait for the parent to finish setup
    if (read(opts->pipe[0], buf, 2) != 2) {
        die(1, "Waiting for pipe read failed");
    }
    if (close(opts->pipe[0]) == -1) {
        die(1, "Failed to close pipe");
    }
