...
class EventSignupForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        def labelize(s, required=True):
            label_class = "class=\"requiredField\"" if required else ""
            asterisk = "<span class=\"asteriskField\">*</span>" if required else ""

            return HTML(f"<label {label_class}>{s}{asterisk}</label>")

        self.helper.layout = Layout(
            Div(
                labelize("Informations de la participante"),
                Row(
            ...
            Div(
                labelize("Informations du responsable légal"),
                Field("phone"),
                *address_applicant_resp,
                Div(
                    Row(
