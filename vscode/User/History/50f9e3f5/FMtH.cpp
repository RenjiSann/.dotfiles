        ...
        for (i32 x = 0; x < width; x++) {
            ...
            for (i32 dx = -MORPHING_RADIUS; dx <= MORPHING_RADIUS; ++dx) {
                for (i32 dy = -MORPHING_RADIUS; dy <= MORPHING_RADIUS; ++dy) {
                    ...
                    // get the input pixel
                    u8 value = GET(in, x + dx, y + dy, width)
                                   ->b1; // Grayscale so r = g = b
                    if (value < min) {
                        min = value;
                    }
                }
...
void apply_overlay(u8 *img, void *mask, u32 height, u32 width, u32 stride) {
    u8_4 *msk = (u8_4 *)mask;

    for (u32 y = 0; y < height; ++y) {
        rgb *buf_line_ptr = (rgb *)(img + y * stride);
        for (u32 x = 0; x < width; ++x) {
            if (GET(msk, x, y, width)->b1 != 0) {
                u32 r = buf_line_ptr[x].r;
                u32 g = buf_line_ptr[x].g;
                u32 b = buf_line_ptr[x].b;
...
void filter_impl(uint8_t *buffer, int width, int height, int stride,
    ...
    */
    ...
    if (bg_img_counter == 0) {
        ...
        // exact size
        binarize(tmp1, tmp2, params->th_low, height, width);

        // Look at tmp1, compute and put high in tmp1
        binarize(tmp1, tmp1, params->th_high, height, width);
