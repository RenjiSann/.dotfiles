...
gst_cuda_filter_class_init (GstCudaFilterClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_TH_LOW,
    g_param_spec_int ("th-low", NULL,
              "Threshold low",
              0, 255, FILTER_DEFAULT_LOW, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property (gobject_class, PROP_TH_HIGH,
    g_param_spec_int ("th-high", NULL,
              "Threshold high",
              0, 255, FILTER_DEFAULT_HIGH, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
}

static void
gst_cuda_filter_init (GstCudaFilter *cudafilter)
{
  cudafilter->th_low = FILTER_DEFAULT_LOW;
  cudafilter->th_high = FILTER_DEFAULT_HIGH;
}
...
gst_cuda_filter_transform_frame_ip (GstVideoFilter * filter, GstVideoFrame * frame)
{
  ...
  struct filter_params params = {
    .th_low = cudafilter->th_low,
    .th_high = cudafilter->th_high
  };

  filter_impl(pixels, width, height, plane_stride, pixel_stride, &params);
