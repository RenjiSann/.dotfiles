...
def merge_jsons(reference, clusters):
    ...
    for i, cluster in enumerate(clusters):
        ...
        ref_vec = np.array(ref["geometry"]["coordinates"], dtype="g")
        cluster_vec = np.array(cluster_image["geometry"]["coordinates"], dtype="g")
        ...
        for feature in cluster.features:
            new_pos = (
                np.array(feature["geometry"]["coordinates"], dtype="g") - delta_vec
            )
            new_pos = list(np.array(new_pos, dtype="double"))
            print(new_pos)
...
def parse_geojson(path: str):
    with open(path, "r") as json_file:
        content = gj.load(json_file, parse_float=np.float128)
        return content
