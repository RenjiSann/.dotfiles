 ...
 */
size_t _gcov_convert_to_gcda(uint8_t *buffer, struct gcov_info *info, struct gcov_summary *summary) {
...
int coverage_dump() {
  ...
  while (tmp) {
    ...
    // if (!buff)
    //   return -ENOMEM;
    size_t size = _gcov_convert_to_gcda(buff, tmp, &summary);

    f = _gcov_open_file(tmp->filename, 0, 0);  // Calls _open in syscalls
    ret = _gcov_write_file(f, buff, size);   // Calls _write in syscalls
    if(ret != size)
      return ret;
...
void set_dump_function(dump_fn fn) {
  _gcov_set_dump_fn(fn);
}
