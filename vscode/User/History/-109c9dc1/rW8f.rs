...
impl Backend {
    async fn on_change(&self, params: TextDocumentItem) {
        ...
        let diagnostics: Vec<Diagnostic> = rope.lines().enumerate().flat_map(|(row, line)| {
            line.chars().enumerate().filter_map(move |(col, c)| {
                MAPPINGS.get(&c).map(|correct| {
                    Diagnostic::new(
                        ...
                        None,
                        None,
                        format!("Character `{}` ({}) could be confused with the ASCII character `{}`({})", c, c.escape_unicode(), correct, correct.escape_unicode()),
                        None,
                        None,
                    )
                })
        ...
        self.diagnostics.insert(params.uri.to_string(), diagnostics.clone());
