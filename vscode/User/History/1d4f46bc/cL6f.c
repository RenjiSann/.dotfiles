...
static int cg_add_controller(const char *cg_path, const char *controller_name);
static int cg_truncated_write(const char *cg_path, const char *fname,
                              const char *to_write);

int cg_init(void) {
    log_info("Initializing cgroup `%s`...", CG_VIRLI);

    // Create a directory, and expect it to fail if already existing.
    int mkdir_ret = mkdir(CG_VIRLI, 0755);
    if (mkdir_ret == -1) {
        if (errno == EEXIST) {
    ...
    if (cg_truncated_write(CG_VIRLI, "memory.max", CG_MAX_MEMORY) != 0) {
    ...
    if (cg_truncated_write(CG_VIRLI, "pids.max", CG_MAX_PIDS) != 0) {
    ...
    if (cg_truncated_write(CG_VIRLI, "cpuset.cpus", CG_MAX_CPU) != 0) {
...
static int cg_truncated_write(const char *cg_path, const char *fname,
    ...
    snprintf(path, PATH_MAX, "%s/%s", cg_path, fname);

    return write_to_file(path, to_write);
}
...
__attribute((unused)) static int
cg_get_controllers(const char *cg_path, char *output, size_t output_len) {
    int res = 0;
    char path[PATH_MAX];
    ...
    snprintf(path, PATH_MAX, "%s/cgroup.controllers", cg_path);
