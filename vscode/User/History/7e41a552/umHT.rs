...
use nom::{
    ...
    IResult,
    error::{ParseError as nomParseError, context}
};
...
fn parse_non_reserved_word<'a>(input: &'a str) -> IResult<&'a str, Word<'a>> {
    let res = verify(parse_word, |x| ! RESERVED_KEYWORDS.contains(&x.text()));
    match res {
        Ok(x) if { RESERVED_KEYWORDS.contains(&x.1.text()) } => context("This is a reserved word", )
        Ok(x) => Ok(x),
        Err(e) => Err(e),
    }
}
...
fn parse_simple_command<'a>(input: &'a str) -> IResult<&'a str, Command<'a>> {
    ...
    map(parser, move |(cmd, args)| Command::new(cmd, args))(input)
}
    ...
    mod parse_simple_command_test {
        ...
        pub fn simple_command_5_words() {
            assert_eq!(
                ...
                Ok(("", Command::new("one", vec!["two", "three", "four", "five"])))
            );
        }
