...
use std::{fs::File, io::Write, path::{Path, PathBuf}, ffi::CStr, str::FromStr, ops::DerefMut};

use anyhow::Result;
use cstr::cstr;
use nix::{
    sys::{utsname::uname, ptrace::{attach, getregs, syscall, setregs}, wait::{waitpid, WaitStatus}},
    unistd::{execvp, fork, ForkResult, Uid, User, Pid, getcwd},
};
...
fn main() {
    ...
    let mut path: PathBuf = getcwd().expect("Failed to get cwd").deref_mut().push(filename);
