 ...
 */
void binarize(u8_4 *in, u8_4 *out, u8 thresh, u32 height, u32 width) {
    for (u32 y = 0; y < height; y++) {

        for (u32 x = 0; x < width; x++) {
            GET(out, x, y, width)->b1 = (GET(in, x, y, width)->b1 > thresh) ? 0xff : 0x00;

            // DEBUG : replicate data for green and blue
            GET(out, x, y, width)->b2 = GET(out, x, y, width)->b1;
            GET(out, x, y, width)->b3 = GET(out, x, y, width)->b1;
        }
    }
}
