...
def merge_jsons(reference, clusters):
    ...
    for (i, cluster) in enumerate(clusters):
        ...
        for feature in cluster.features:
            geo = gj.Point(list(np.array(feature["geometry"]["coordinates"]) - delta_vec))
            feat = gj.Feature(geometry=geo, properties=feature["properties"])
