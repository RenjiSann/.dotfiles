...
int trace_scheduling_done(struct pt_regs *ctx, struct task_struct *prev)
{
    ...
    // Override max-latency if needed
    if (delta_ns > data->max_delta) {
        data->max_delta = delta_ns;
    }

    // Compute avg
    // nb_sched counts the current scheduling.
    if (data->nb_sched == 1) {
        data->avg_delta = delta_ns;
    } else {
        data->avg_delta = (data->avg_delta * (data->nb_sched - 1) + delta_ns) / data->nb_sched;
    }
