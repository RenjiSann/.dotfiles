...
static int is_in_array(cap_value_t to_test, const cap_value_t *to_keep, size_t nb_capa) {
    for (size_t i = 0; i < nb_capa; i++) {
        if (to_test == to_keep[i]) return 1;
    }
    return 0;
}

int capa_init_fork(void) {
    const cap_value_t capas_to_add[] = { CAP_NET_RAW, CAP_SYS_CHROOT };
    size_t nb_capa = 2;

    // 1. Clear the inheritable set, we don't want to inherit random capabilities.
    ...
    if (cap_set_flag(cap, CAP_INHERITABLE, nb_capa, capas_to_add, CAP_SET) != 0) {
    ...
    // Iterate through all capabilities to frop them
