...
void dump_opts(moulette_opts *opts) {
    ...
    if (opts->student_directory != NULL) {
    fprintf(stderr, "    Student directory: `%s`\n", opts->student_directory);
    }

    // Print entrypoint
    fprintf(stderr, "    Entrypoint:");
...
int check_opts(const moulette_opts *opts) {
    ...
    if (strlen(opts->path) == 0) {
        log_error("[ERROR] %s cannot be empty",
                  (opts->kind == MOULETTE_ROOTFS) ? "rootfs" : "docker image");
        res = 1;
    }
    if (strlen(opts->student_directory) == 0) {
        fprintf(stderr, "[ERROR] student directory path cannot be empty\n");
        res = 1;
    }
