...
int set_user_mappings(pid_t pid) {
    char buf[128];

    snprintf(buf, 128, "/proc/%d/uid_map", pid);
    int uid_map_fd = open(buf, O_WRONLY);
    if (uid_map_fd == -1) {
        die(1, "Cannot set uid_map: %s", strerror(errno));
    }
    if (dprintf(uid_map_fd, "0 %d 1\n", 1000) < 0) {
        die(1, "Cannot set uid_map: %s", strerror(errno));
    }
    close(uid_map_fd);

    snprintf(buf, 128, "/proc/%d/setgroups", pid);
    int setgroups = open(buf, O_WRONLY);
    if (setgroups == -1) {
        die(1, "Cannot setgroups: %s", strerror(errno));
    }
    if (dprintf(setgroups, "deny\n", 1000) < 0) {
        die(1, "Cannot setgroups: %s", strerror(errno));
    }
    close(setgroups);

    snprintf(buf, 128, "/proc/%d/gid_map", pid);
    int gid_map_fd = open(buf, O_WRONLY);
    if (gid_map_fd == -1) {
        die(1, "Cannot set gid_map: %s", strerror(errno));
    }
    if (dprintf(gid_map_fd, "0 %d 1\n", 1000) < 0) {
        die(1, "Cannot set gid_map: %s", strerror(errno));
    }
    close(gid_map_fd);
...
int main(int argc, char **argv) {
    ...
    if (opts.verbose_level < LOG_INFO) {
        // Only print capabilities when DEBUG
        print_effective();
        print_permitted();
        print_inheritable();
        print_bounding();
    ...
    if (pid == -1) {
        // Error during clone
        log_error("Error when cloning: %s", strerror(errno));
        return 1;
    }
    // Parent process
    set_user_mappings(pid);
...
// int uidmap = open("/proc/self/uid_map", O_WRONLY);
// if (uidmap == -1) {

// }
// dprintf(uidmap, "0 %d 1\n", uid);
// close(uidmap);

// if (setuid(0) != 0) {
//     log_error("Failed setuid: %s", strerror(errno));
//     return 1;
// }
// // if (setgid(0) != 0) {
// //     log_error("Failed setgid: %s", strerror(errno));
// //     return 1;
// // }