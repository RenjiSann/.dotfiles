...
pub fn string_to_int_roman(s: String) -> i32 {
    ...
    loop {
        if let Some(c) = chars.next() {
            ...
            if let Some(next) = chars.peek() {
                if sign.value() < Sign::from(*next).value() {
                    res -= sign.value();
                } else {
                    res += sign.value();
                }
...
mod tests {
    ...
    #[test]
    fn test_string_to_int_roman() {
        assert_eq!(string_to_int_roman("XV".to_string()), 15);
        assert_eq!(string_to_int_roman("XCIX".to_string()), 99);
        assert_eq!(string_to_int_roman("MMMCMXCIX".to_string()), 3999);
    }
}
