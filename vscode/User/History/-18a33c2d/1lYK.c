...
static void prepare_mntns(const char *rootfs)
{
    const char *mnt = rootfs;

    if (mount(rootfs, mnt, "ext4", MS_BIND, ""))
        die(1, "Failed to mount %s at %s: %m\n", rootfs, mnt);

    if (chdir(mnt))
        die(1, "Failed to chdir to rootfs mounted at %s: %m\n", mnt);

    const char *put_old = ".put_old";
    if (mkdir(put_old, 0777) && errno != EEXIST)
        die(1, "Failed to mkdir put_old %s: %m\n", put_old);

    if (syscall(SYS_pivot_root, ".", put_old))
        die(1, "Failed to pivot_root from %s to %s: %m\n", rootfs, put_old);

    if (chdir("/"))
        die(1, "Failed to chdir to new root: %m\n");

    mount_proc();

    if (umount2(put_old, MNT_DETACH))
        die(1, "Failed to umount put_old %s: %m\n", put_old);
}
...
static int mount_proc(void) {
    ...
    if (mount("proc", "/proc", "proc", 0, "")) {
        log_error("Failed to mount proc: %d", errno);
