...
void erode(void *in, void *out, u32 height, u32 width) {
    u8_4 *i = (u8_4 *)in;
    u8_4 *o = (u8_4 *)out;
        ...
        for (i32 x = 0; x < width; x++) {
            ...
            for (i32 dx = -MORPHING_RADIUS; dx <= MORPHING_RADIUS; ++dx) {
                for (i32 dy = -MORPHING_RADIUS; dy <= MORPHING_RADIUS; ++dy) {
                    ...
                    // get the input pixel
                    u8 value = GET(i, x + dx, y + dy, width)->b1;
                    if (value < min) {
                        min = value;
                    }
                }
...
void dilate(void *in, void *out, u32 height, u32 width) {
    u8_4 *i = (u8_4 *)in;
    u8_4 *o = (u8_4 *)out;
...
void copy_to_strided_buffer(void *in, u8 *out, u32 height, u32 width,
                            u32 stride) {
    u8_4 *i = (u8_4 *)in;
...
void debug_show_low_on_high(void *high, void *low, u32 height, u32 width) {
    ...
    for (u32 y = 0; y < height; ++y) {
        for (u32 x = 0; x < width; ++x) {
            auto low_ptr = GET(l, x, y, width);
            auto high_ptr = GET(l, x, y, width);

            low_ptr->b1 &= high_ptr->b1;
            low_ptr->b2 ^= high_ptr->b2;
            low_ptr->b3 ^= 0;
        }
    }
}
