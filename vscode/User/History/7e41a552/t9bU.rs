...
use nom::{
    ...
    bytes::complete::tag,
    character::complete::{
        alpha1, alphanumeric1, char, newline, space0, space1, multispace0,
    },
...
fn parse_non_reserved_word<'a>(input: &'a str) -> IResult<&'a str, Word<'a>> {
    ...
    match res {
        Ok(x) if {RESERVED_KEYWORDS.contains(&x.1.text()) } => Err(crate::ParseError::new(format!("{} is a reserved word", x.1.text()))
        Ok(x) => Ok(x),
        Err(e) => Err(e)
    }
}
...
fn parse_simple_command<'a>(input: &'a str) -> IResult<&'a str, Command<'a>> {
    let parser = preceded(space0, pair(parse_word));
    map(separated_list1(space1, parse_word), move |v| v.into())(input)
}
