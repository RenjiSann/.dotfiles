procedure Exo4 is
    ...
    -- (X L C)
    -- (C D M)
    function DigitToString(d: Digit; c1: Character; c2: Character; c3: Character) return String is
        ret: String(1 .. 4);
        size: Integer := 0;
    begin
        case d is
            when 0 => return "";
            when 1 => ret(1) := c1; size := 1;
            when 2 => ret(1) := c1; ret(2) := c1; size := 2;
            when 3 => ret(1) := c1; ret(2) := c1; ret(3) := c1; size := 3;
            when 4 => ret(1) := c1; ret(2) := c2; size := 2;
            when 5 => ret(1) := c2; size := 1;
            when 6 => ret(1) := c2; ret(2) := c1; size := 2;
            when 7 => ret(1) := c2; ret(2) := c1; ret(3) := c1; size := 3;
            when 8 => ret(1) := c2; ret(2) := c1; ret(3) := c1; ret(4) := c1; size := 4;
            when 9 => ret(1) := c1; ret(2) := c3; size := 2;
        end case;
